...
#define WHITE gameTemp::Color(255, 255, 255, 255)
#define GRAY gameTemp::Color(128, 128, 128, 255)
#define BLACK gameTemp::Color(0, 0, 0, 255)
#define RED gameTemp::Color(255, 0, 0, 255)
#define GREEN gameTemp::Color(0, 255, 0, 255)
#define BLUE gameTemp::Color(0, 0, 255, 255)
#define MAGENTA gameTemp::Color(255, 0, 255, 255)
#define YELLOW gameTemp::Color(255, 255, 0, 255)
#define CYAN gameTemp::Color(0, 255, 255, 255)

bool gameTemp::mesh_utils::loadOBJ(gameTemp::Mesh &mesh, const char *filename)
{
    ...
    std::unordered_map<gameTemp::Vertex, GLuint> vertex_map;
    ...
    if (mesh.isCreated())
        mesh.destroy();
    mesh.create({gameTemp::setup_buffer_accessors<Vertex>});
    mesh.setVertexData(0, vertices);
    mesh.setElementData(elements);
    return true;
}

void gameTemp::mesh_utils::Cuboid(Mesh &mesh,
                                  bool colored_faces,
                                  ...
                                  const glm::vec2 &texture_offset, const glm::vec2 &texture_tiling)
{
    ...
    for (int lat = 0; lat <= segments.y; lat++)
    {
        ...
        for (int lng = 0; lng <= segments.x; lng++)
        {
            ...
            gameTemp::Color color = colored ? gameTemp::Color(127.5f * (normal + 1.0f), 255) : WHITE;
...
void gameTemp::mesh_utils::Plane(gameTemp::Mesh &mesh, const glm::ivec2 &resolution, bool colored,
                                 ...
                                 const glm::vec2 &texture_offset, const glm::vec2 &texture_tiling)
{
    ...
    glm::ivec2 it;
    glm::vec3 position = {0, center.y, 0};
    glm::vec2 uv;
    for (it.x = 0; it.x <= resolution.x; it.x++)
    {
        ...
        for (it.y = 0; it.y <= resolution.y; it.y++)
        {
            ...
            gameTemp::Color color = colored ? glm::mix(
                                                  glm::mix(gameTemp::Color(255, 0, 0, 255), gameTemp::Color(0, 255, 0, 255), uv.s),
                                                  glm::mix(gameTemp::Color(255, 255, 0, 255), gameTemp::Color(0, 0, 255, 255), uv.s),
                                                  uv.t)
                                            : WHITE;
