...
void gameTemp::Texture::create(GLuint texture,const char *filename, bool Active_Mipmap, int mipmapLevel,
                               int unpack )
{
...
void gameTemp::Texture::create(GLuint texture, bool Active_Mipmap, int mipmapLevel, int width, int height, int unpack,
                               uint8_t *pixel_data) {
    this->texture = texture;
    this-> Active_Mipmap= Active_Mipmap;
    this->mipmapLevel = mipmapLevel;
    ...
    glBindTexture(GL_TEXTURE_2D, this->texture);
    glPixelStorei(GL_UNPACK_ALIGNMENT, this->unpack);
    glTexImage2D(GL_TEXTURE_2D, this->mipmapLevel, GL_RGBA8, this->width, this->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixel_data);
    if(this->Active_Mipmap){
        glGenerateMipmap(GL_TEXTURE_2D);
    }
